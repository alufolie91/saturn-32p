...
boolean LUAh_KartHyudoro(player_t *player, INT32 *target, boolean sink)
{
	hook_p hookp;
	boolean forceSink = false;
	if (!gL || !(hooksAvailable[hook_KartHyudoro/8] & (1<<(hook_KartHyudoro%8))))
		return 0;

	lua_settop(gL, 0);

	for (hookp = roothook; hookp; hookp = hookp->next)
		if (hookp->type == hook_KartHyudoro)
		{
			if (lua_gettop(gL) == 0)
			{
				LUA_PushUserdata(gL, player, META_PLAYER); // Player that is stealing
				if (*(target) == -1)
					lua_pushnil(gL); // Pass nil if the player doesn't exist.
				else
					LUA_PushUserdata(gL, &players[*(target)], META_PLAYER); // Player being stolen from
				lua_pushboolean(gL, sink);	// Was a sink rolled?
			}
			...
			if (lua_isuserdata(gL, -2))
				*(target) = lua_tonumber(gL, -2);

			if (lua_isboolean(gL, -1))
				forceSink = lua_toboolean(gL, -1);
...
boolean LUAh_KartStealBumper(player_t *player, player_t *target, boolean *force)
{
	hook_p hookp;
	boolean stop = false;
	if (!gL || !(hooksAvailable[hook_KartStealBumper/8] & (1<<(hook_KartStealBumper%8))))
		return 0;

	lua_settop(gL, 0);

	for (hookp = roothook; hookp; hookp = hookp->next)
		if (hookp->type == hook_KartStealBumper)
		{
			if (lua_gettop(gL) == 0)
			{
				LUA_PushUserdata(gL, player, META_PLAYER); // Player that is stealing
				LUA_PushUserdata(gL, target, META_PLAYER); // Player being stolen from
				lua_pushboolean(gL, *(force));	// Are we forcing this burglary?
			}
			...
			if (lua_isboolean(gL, -2))
				stop = lua_toboolean(gL, -2);

			if (lua_isboolean(gL, -1))
				*(force) = lua_toboolean(gL, -1);
...
boolean LUAh_KartSneaker(player_t *player, int type)
{
	hook_p hookp;
	boolean stop = false;
	if (!gL || !(hooksAvailable[hook_KartSneaker/8] & (1<<(hook_KartSneaker%8))))
		return 0;

	lua_settop(gL, 0);

	for (hookp = roothook; hookp; hookp = hookp->next)
		if (hookp->type == hook_KartSneaker)
		{
			if (lua_gettop(gL) == 0)
			{
				LUA_PushUserdata(gL, player, META_PLAYER); // Player that is stealing
				// Type of sneaker: 0 - perfect/panel, 1 - regular sneaker, 2 - rocket sneaker.
				// Only changes visuals as far as I'm concerned.
...
boolean LUAh_PlayerItemUse(player_t *player, kartitems_t itemType, boolean wasHoldingItem, boolean *force)
{
	hook_p hookp;
	boolean hooked = false;
	if (!gL || !(hooksAvailable[hook_PlayerItemUse/8] & (1<<(hook_PlayerItemUse%8))))
		return 0;
	...
	for (hookp = roothook; hookp; hookp = hookp->next)
		if (hookp->type == hook_PlayerItemUse)
		{
			if (lua_gettop(gL) == 0)
			{
				...
				lua_pushboolean(gL, wasHoldingItem); // Was the playing dragging/orbiting this?
			}
...
// Hook for P_SetScale by mobj type
boolean LUAh_MobjScaleChange(mobj_t *target, fixed_t newscale, fixed_t oldscale)
{
	hook_p hookp;
	boolean hooked = false;
	if (!gL || !(hooksAvailable[hook_MobjScaleChange/8] & (1<<(hook_MobjScaleChange%8))))
		return 0;
	...
	// Look for all generic mobj death hooks
	for (hookp = mobjhooks[MT_NULL]; hookp; hookp = hookp->next)
		if (hookp->type == hook_MobjScaleChange)
		{
			lua_pushfstring(gL, FMT_HOOKID, hookp->id);
			lua_gettable(gL, LUA_REGISTRYINDEX);
